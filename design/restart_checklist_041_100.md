...
4. GitHub 推送必须命中章号锚点：`45/50/55/.../100`，不得改成滑动批次。
...
使用 `scripts/count_detailed.py` 进行统计：
...
| 章型 | 汉字要求 | 不达标处理 |
|------|----------|------------|
| normal | <3500 返回重写；3500-7000 可放行；>5500 记 WARN | 返回重写 |
| key | <5000 返回重写；5000-8000 可放行；>6500 记 WARN | 返回重写 |
...
- [ ] 写作模型是否与章型一致
- [ ] **字数是否达标**（normal 不低于 `3500`、key 不低于 `5000`；超出建议上限需记录 `WARN`）
- [ ] 关键人物音色是否稳定
